...
double Interference::ComputeInterference(UserEquipment* ue) {
  ENodeB* node;
...
#ifdef UE_DEBUG
  std::cout << "Compute interference for UE " << ue->GetIDNetworkNode()
            << " ,target node " << ue->GetTargetNode()->GetIDNetworkNode()
            << std::endl;
#endif
  ...
  for (it2 = HeNBs->begin(); it2 != HeNBs->end(); it2++) {
    ...
    if (henb->GetIDNetworkNode() != ue->GetTargetNode()->GetIDNetworkNode() &&
      ...
      double nodeInterference_db =
          powerTXForSubBandwidth -
          ComputePathLossForInterference(henb, ue);  // in dB
      double nodeInterference = pow(10, nodeInterference_db / 10);
