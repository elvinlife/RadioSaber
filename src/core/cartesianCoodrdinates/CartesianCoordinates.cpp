...
void CartesianCoordinates::SetCoordinates(double x, double y) {
  m_posX = x;
  m_posY = y;
  m_posZ = 0.0;
}

void CartesianCoordinates::SetCoordinates(double x, double y, double z) {
  m_posX = x;
  m_posY = y;
  m_posZ = z;
}

void CartesianCoordinates::SetCoordinatesFromPolar(CartesianCoordinates* center,
                                                   double radius,
                                                   double angle) {
...
double CartesianCoordinates::GetDistance(CartesianCoordinates* remoteObject) {
  /*
...
double CartesianCoordinates::GetDistance(double remoteObjectPosX,
                                         double remoteObjectPosY) {
  /*
   * Computes distance from this object and remoteObject
   */
  return sqrt(pow((GetCoordinateX() - remoteObjectPosX), 2) +
              pow((GetCoordinateY() - remoteObjectPosY), 2));
}

double CartesianCoordinates::GetPolarAzimut(CartesianCoordinates* center) {
...
void CartesianCoordinates::Print() {
  std::cout << "\t CenterCellPosition: "
               "x = "
            << GetCoordinateX() << ", y = " << GetCoordinateY() << std::endl;
}
