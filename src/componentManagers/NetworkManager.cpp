...
NetworkManager::~NetworkManager() {
  ...
  for (iter = m_cellContainer->begin(); iter != m_cellContainer->end();
       iter++) {
    delete *iter;
  }
  ...
  for (iter4 = m_gatewayContainer->begin(); iter4 != m_gatewayContainer->end();
       iter4++) {
    delete *iter4;
  }
  ...
  for (iter2 = m_eNodeBContainer->begin(); iter2 != m_eNodeBContainer->end();
       iter2++) {
    delete *iter2;
  }
...
Cell* NetworkManager::CreateCell(int idCell, double radius, double minDistance,
                                 double X, double Y) {
...
ENodeB* NetworkManager::CreateEnodeb(int id, Cell* cell, double pos_X,
                                     double pos_Y, LteChannel* dlCh,
                                     LteChannel* ulCh, BandwidthManager* bm) {
...
Gateway* NetworkManager::CreateGateway(void) {
...
UserEquipment* NetworkManager::CreateUserEquipment(int id, double pos_X,
                                                   double pos_Y, double speed,
                                                   double speedDirection,
                                                   Cell* cell, ENodeB* enb) {
  UserEquipment* ue =
      new UserEquipment(id, pos_X, pos_Y, speed, speedDirection, cell, enb, 0,
...
void NetworkManager::CreateStreet(int idStreet, CartesianCoordinates* center,
                                  double theta, int buildingtype,
                                  int nbBuildings, double street_width,
                                  double building_distance,
                                  int nbFemtoInBuilding, double apartment_side,
                                  int firstFemtoIDInStreet) {
  Street* street =
      new Street(idStreet, center, theta, nbBuildings, buildingtype,
                 street_width, building_distance, apartment_side);

  vector<CartesianCoordinates*>* building_positions =
      street->GetBuildingDistributionInStreet();
  ...
  while (idBuilding < 2 * nbBuildings) {
    ...
    CreateBuildingForFemtocells(idBuilding, buildingtype, apartment_side, 1,
...
void NetworkManager::CreateBuildingForFemtocells(
    int idBuilding, int buildingType, double apartmentSide, int nbFloors,
    double pos_X, double pos_Y, int firstFemtoCellID, int nbOfFemtocells) {
  ...
  Building* building = new Building(idBuilding, buildingType, nbFloors, side_X,
                                    side_Y, pos_X, pos_Y);
  ...
  for (int floor = 0; floor < nbFloors; floor++) {
    ...
    while (femtocellIndex < nbOfFemtocells) {
      ...
      double* offset = GetFemtoCellOffsetInBuilding(
          building->GetBuildingType(), femtocellIndex, apartmentSide);

      building->AddFemtoCell(CreateFemtoCell(
          idCell, apartmentSide, offset[0] + pos_X, offset[1] + pos_Y));
...
Femtocell* NetworkManager::CreateFemtoCell(int idCell, double side, double X,
                                           double Y) {
...
int NetworkManager::GetNbCell(void) {
...
std::vector<Femtocell*>* NetworkManager::GetFemtoCellContainer(void) {
...
std::vector<HeNodeB*>* NetworkManager::GetHomeENodeBContainer(void) {
...
std::vector<Gateway*>* NetworkManager::GetGatewayContainer(void) {
  return m_gatewayContainer;
}

std::vector<Building*>* NetworkManager::GetBuildingContainer(void) {
...
Cell* NetworkManager::GetCellByID(int idCell) {
  ...
  std::vector<Cell*>::iterator iter;
  Cell* cell;
  for (iter = cellContainer->begin(); iter != cellContainer->end(); iter++) {
    cell = *iter;
...
Femtocell* NetworkManager::GetFemtoCellByID(int idFemtoCell) {
  std::vector<Femtocell*>* femtocellContainer = GetFemtoCellContainer();
  std::vector<Femtocell*>::iterator iter;
  Femtocell* cell;
  for (iter = femtocellContainer->begin(); iter != femtocellContainer->end();
       iter++) {
    cell = *iter;
...
ENodeB* NetworkManager::GetENodeBByID(int idENodeB) {
  ...
  std::vector<ENodeB*>::iterator iter2;
  ENodeB* eNodeB;
  for (iter2 = eNodeBContainer->begin(); iter2 != eNodeBContainer->end();
       iter2++) {
    eNodeB = *iter2;
...
Gateway* NetworkManager::GetGatewayByID(int idGW) {
  std::vector<Gateway*>* gatewayContainer = GetGatewayContainer();
  std::vector<Gateway*>::iterator iter;
  Gateway* gateway;

  for (iter = gatewayContainer->begin(); iter != gatewayContainer->end();
       iter++) {
    gateway = *iter;
...
Building* NetworkManager::GetBuildingByID(int idBuilding) {
  std::vector<Building*>* buildingContainer = GetBuildingContainer();
  std::vector<Building*>::iterator iter;
  Building* building;

  for (iter = buildingContainer->begin(); iter != buildingContainer->end();
       iter++) {
    building = *iter;
...
Building* NetworkManager::GetBuildingByFemtoCellID(int idFemtoCell) {
  std::vector<Building*>* buildingContainer = GetBuildingContainer();
  std::vector<Building*>::iterator iter;
  Building* building;
...
std::vector<UserEquipment*>* NetworkManager::GetRegisteredUEToENodeB(
    int idENB) {
...
std::vector<int> NetworkManager::GetCellIDFromPosition(
  ...
  for (iter = cellContainer->begin(); iter != cellContainer->end(); iter++) {
    cell = *iter;
    distance = sqrt(pow((cell->GetCellCenterPosition()->GetCoordinateX() -
                         position->GetCoordinateX()),
                        2) +
                    pow((cell->GetCellCenterPosition()->GetCoordinateY() -
...
int NetworkManager::GetCellIDFromPosition(double posX, double posY) {
  std::vector<Cell*>* cellContainer = GetCellContainer();
  std::vector<Cell*>::iterator iter;
  Cell* cell;
...
void NetworkManager::UpdateUserPosition(double time) {
  ...
  std::cout << "MOBILITY_DEBUG: UPDATE POSITION, "
               "number of UE = "
            << records->size() << " time = " << time << std::endl;
#endif
  ...
  for (iter = records->begin(); iter != records->end(); iter++) {
...
#ifdef MOBILITY_DEBUG
    std::cout << "\t USER  " << record->GetIDNetworkNode() << std::endl;
#endif
    ...
    if (record->GetMobilityModel()->GetHandover() == true) {
      ...
      if (targetNode->GetProtocolStack()
        ...
        std::cout << "** HO ** \t time: " << time << " user "
                  << record->GetIDNetworkNode() << " old eNB "
                  << targetNode->GetIDNetworkNode() << " new eNB "
                  << newTagertNode->GetIDNetworkNode() << std::endl;
#endif
...
bool NetworkManager::CheckHandoverPermissions(NetworkNode* target,
                                              UserEquipment* user) {
  if (target->GetNodeType() == NetworkNode::TYPE_ENODEB) {
    ...
    HenbMacEntity* henb_mac =
        (HenbMacEntity*)target->GetProtocolStack()->GetMacEntity();
...
bool NetworkManager::CheckIndoorUsers(UserEquipment* ue) {
  std::vector<Building*>* buildingContainer = GetBuildingContainer();
  std::vector<Building*>::iterator iter;
  Building* building;
  ...
  if (buildingContainer->size() > 0) {
    for (iter = buildingContainer->begin(); iter != buildingContainer->end();
      ...
      if ((abs(building->GetCenterPosition()->GetCoordinateX() -
               ue->GetMobilityModel()
                   ->GetAbsolutePosition()
                   ->GetCoordinateX()) < building->GetSide()[0] / 2) &&
          (abs(building->GetCenterPosition()->GetCoordinateY() -
               ue->GetMobilityModel()
                   ->GetAbsolutePosition()
                   ->GetCoordinateY()) < building->GetSide()[1] / 2)) {
        return true;
      }
    }
  }
...
int NetworkManager::GetBuildingIDForUE(UserEquipment* ue) {
  std::vector<Building*>* buildingContainer = GetBuildingContainer();
  std::vector<Building*>::iterator iter;
  Building* building;
...
Cell* NetworkManager::GetBelongingCellFromPosition(UserEquipment* ue) {
  ...
  if (buildingID != -1) {
    ...
    std::vector<Femtocell*>* femtocellContainer =
        GetBuildingByID(buildingID)->GetFemtoCellsInBuilding();
    std::vector<Femtocell*>::iterator iter;
    Femtocell* cell;
    ...
    return GetCellByID(GetCellIDFromPosition(ue_pos->GetCoordinateX(),
                                             ue_pos->GetCoordinateY()));
  }

  return NULL;
}

void NetworkManager::SelectTargetNode(UserEquipment* ue) {
  NetworkNode* targetNode = ue->GetTargetNode();

  if (targetNode->GetProtocolStack()
          ->GetRrcEntity()
          ->GetHandoverEntity()
          ->CheckHandoverNeed(ue)) {
    NetworkNode* newTagertNode = targetNode->GetProtocolStack()
                                     ->GetRrcEntity()
                                     ->GetHandoverEntity()
                                     ->GetHoManager()
...
void NetworkManager::HandoverProcedure(double time, UserEquipment* ue,
                                       NetworkNode* oldTarget,
...
#ifdef MOBILITY_DEBUG
  std::cout << "user " << ue->GetIDNetworkNode()
            << " starts HO "
               "\n\t old eNB = "
            << ue->GetTargetNode()->GetIDNetworkNode()
            << "\n\t target eNB = " << newTarget->GetIDNetworkNode()
            << std::endl;
#endif

  // 2 - transfer all active radio bearer
  Simulator::Init()->Schedule(0.001, &NetworkManager::TransferBearerInfo, this,
                              ue, newTarget);

  ue->GetMobilityModel()->SetLastHandoverTime(time);
}

void NetworkManager::TransferBearerInfo(UserEquipment* ue,
                                        NetworkNode* target) {
#ifdef HANDOVER_DEBUG
  ...
  if ((target->GetNodeType() == NetworkNode::TYPE_ENODEB &&
      ...
      (target->GetNodeType() == NetworkNode::TYPE_HOME_BASE_STATION &&
       ue->GetTargetNode()->GetNodeType() == NetworkNode::TYPE_ENODEB) ||
      (target->GetNodeType() == NetworkNode::TYPE_ENODEB &&
       ue->GetTargetNode()->GetNodeType() ==
...
#ifdef HANDOVER_DEBUG
    std::cout << "update spectrum, channels and propagation loss model"
              << std::endl;
#endif
    ue->GetPhy()->SetBandwidthManager(
        newTargetNode->GetPhy()->GetBandwidthManager());
    ...
    if (newDl->GetPropagationLossModel() != NULL) {
      newDl->GetPropagationLossModel()->AddChannelRealization(
          CreateChannelRealization(newTargetNode, ue));

      oldDl->GetPropagationLossModel()->DelChannelRealization(oldTargetNode,
                                                              ue);
    }
    ...
    if (newUl->GetPropagationLossModel() != NULL) {
      ...
      oldUl->GetPropagationLossModel()->DelChannelRealization(ue,
                                                              oldTargetNode);
      ;
    }
    ...
    // --> move dl bearers
#ifdef HANDOVER_DEBUG
    std::cout << "\t DL radio bearers "
              << oldEnbRrc->GetRadioBearerContainer()->size() << std::endl;
#endif
    std::vector<RadioBearer*>* dlBearerToDelete =
        new std::vector<RadioBearer*>();
    for (std::vector<RadioBearer*>::iterator it =
             oldEnbRrc->GetRadioBearerContainer()->begin();
         it != oldEnbRrc->GetRadioBearerContainer()->end(); it++) {
      RadioBearer* bearer = (*it);
      if (bearer->GetDestination()->GetIDNetworkNode() ==
          ue->GetIDNetworkNode()) {
...
#endif
    std::vector<RadioBearerSink*>* ulBearerToDelete =
        new std::vector<RadioBearerSink*>();
    for (std::vector<RadioBearerSink*>::iterator it =
             oldEnbRrc->GetRadioBearerSinkContainer()->begin();
...
#endif
    for (std::vector<RadioBearerSink*>::iterator it =
             ueRrc->GetRadioBearerSinkContainer()->begin();
...
#endif
    for (std::vector<RadioBearer*>::iterator it =
             ueRrc->GetRadioBearerContainer()->begin();
...
std::vector<BandwidthManager*> RunFrequencyReuse(int nodes, int cluster,
                                                 double bandwidth) {
  RunFrequencyReuseTechniques(nodes, cluster, bandwidth);
}

void NetworkManager::PrintUEsForEachCell(void) {
 ...
 *  DEBUG
 */
void NetworkManager::Print(void) {
...
void NetworkManager::Print(NetworkManager::m_debugInfo info) {
  ...
  switch (info) {
    ...
    case NetworkManager::ONLY_CELL_DETAILS: {
      ...
      std::vector<Cell*>::iterator iter;
      Cell* cell;
      for (iter = cellContainer->begin(); iter != cellContainer->end();
           iter++) {
        cell = *iter;
        cell->Print();
      }
...
void NetworkManager::PrintFrequencyMask(void) {
  std::vector<Cell*>* cellContainer = GetCellContainer();
  std::vector<Cell*>::iterator iter;
  Cell* cell;
  for (iter = cellContainer->begin(); iter != cellContainer->end(); iter++) {
  ...
  for (iter2 = eNodeBContainer->begin(); iter2 != eNodeBContainer->end();
       iter2++) {
    eNodeB = *iter2;
    std::cout << "ENODEB " << eNodeB->GetIDNetworkNode()
              << ", frequencyMask:" << std::endl;
  }
}

void NetworkManager::PrintUserPosition(void) {
  ...
  for (iter = users->begin(); iter != users->end(); iter++) {
    user = *iter;
    std::cout
        << user->GetMobilityModel()->GetAbsolutePosition()->GetCoordinateX()
        << " ";
  }
  ...
  for (iter = users->begin(); iter != users->end(); iter++) {
    user = *iter;
    std::cout
        << user->GetMobilityModel()->GetAbsolutePosition()->GetCoordinateY()
        << " ";
  }
  std::cout << std::endl;
}
