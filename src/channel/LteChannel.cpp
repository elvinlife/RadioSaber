...
void LteChannel::StartTx(PacketBurst* p, TransmittedSignal* txSignal,
                         NetworkNode* src) {
  ...
  Simulator::Init()->Schedule(0.001, &LteChannel::StartRx, this, p, txSignal,
                              src);
...
void LteChannel::StartRx(PacketBurst* p, TransmittedSignal* txSignal,
                         NetworkNode* src) {
#ifdef TEST_DEVICE_ON_CHANNEL
...
#ifdef TEST_DEVICE_ON_CHANNEL
    std::cout << "\t Node " << dst->GetIDNetworkNode() << " is attached"
              << std::endl;
#endif
...
#ifdef TEST_DEVICE_ON_CHANNEL
  std::cout << "LteChannel " << m_channelId << " ADD Node "
            << d->GetIDNetworkNode() << std::endl;
#endif
...
#ifdef TEST_DEVICE_ON_CHANNEL
  std::cout << "LteChannel " << m_channelId << " DEL Node "
            << d->GetIDNetworkNode() << std::endl;
#endif
...
bool LteChannel::IsAttached(NetworkNode* d) {
  for (std::vector<NetworkNode*>::iterator it = GetDevices()->begin();
       it != GetDevices()->end(); it++) {
    NetworkNode* node = *it;
    ...
    if (node->GetIDNetworkNode() == d->GetIDNetworkNode()) {
#ifdef TEST_DEVICE_ON_CHANNEL
      std::cout << "LteChannel find Node " << d->GetIDNetworkNode()
                << std::endl;
#endif
      return true;
    }
