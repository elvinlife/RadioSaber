...
bool ChannelRealization::NeedForUpdate(void) {
  ...
  if (Simulator::Init()->Now() >=
      (GetLastUpdate() + GetSamplingPeriod()) - 0.001) {
    return true;
  } else {
    return false;
  }
...
void ChannelRealization::SetChannelType(ChannelType t) {
  m_channelType = t;
  //UpdateFastFading ();
}

ChannelRealization::ChannelType ChannelRealization::GetChannelType(void) {
  return m_channelType;
}

ChannelRealization::FastFading* ChannelRealization::GetFastFading(void) {
  return m_fastFading;
}

void ChannelRealization::SetFastFading(ChannelRealization::FastFading* ff) {
  m_fastFading = ff;
}

void ChannelRealization::UpdateFastFading(void) {
  ...
  int numbOfSubChannels = GetSourceNode()
                              ->GetPhy()
                              ->GetBandwidthManager()
                              ->GetDlSubChannels()
                              .size();
  ...
  if (GetChannelType() == ChannelRealization::CHANNEL_TYPE_JAKES) {
    ...
    std::cout << "UpdateFastFading, "
              ...
              << "\n\t start_point_time " << start_point_time << std::endl;
#endif
