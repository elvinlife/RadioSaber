...
static void SingleCellCustomize (int nbCells, double radius,
    int nbUE,
    int nbVoIP, int nbVideo, int nbBE, int nbCBR,
    int sched_type,
    int frame_struct,
    int speed,
    double maxDelay, string video_trace,
    int seed,
    string config_fname)
{
  ...
  int nbCell=1;
  VoIP VoIPApplication[nbVoIP*nbCell];
  TraceBased VideoApplication[nbVideo*nbCell];
  InfiniteBuffer BEApplication[nbBE*nbCell];
  CBR CBRApplication[nbCBR*nbCell];
  ...
  {
    UserEquipment* ue = genNewUE(idUE, radius, speed, cells->at(0), eNBs->at(0), nm);
