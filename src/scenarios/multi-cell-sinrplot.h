...
static void MultiCellSinrPlot(int nbCell, double radius, int nbUE,
                              int sched_type, int frame_struct, int speed,
                              int mobility_model, int seed) {
  int handover;
  if (nbCell > 1)
    handover = 1;  // if multicell scenario -> activate HO mechanisms
  ...
  for (int i = 0; i < nbUE; i++) {
    ...
    UserEquipment *ue =
        new UserEquipment(idUE, posX, posY, speed, speedDirection, cells->at(0),
                          eNBs->at(0), handover, model);
