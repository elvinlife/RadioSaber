...
void MacQueue::SetMaxSize(int maxSize) {
  m_maxSize = maxSize;
}

int MacQueue::GetMaxSize(void) const {
  return m_maxSize;
}

void MacQueue::SetQueueSize(int size) {
  m_queueSize = size;
}

void MacQueue::UpdateQueueSize(int packetSize) {
  m_queueSize += packetSize;
}

void MacQueue::UpdateNbDataPackets(void) {
  m_nbDataPackets += 1;
}

int MacQueue::GetNbDataPackets(void) const {
  return m_nbDataPackets;
}

int MacQueue::GetQueueSize(void) const {
  return m_queueSize;
}

int MacQueue::GetQueueSizeWithMACHoverhead(void) const {
  if (!IsEmpty()) {
...
MacQueue::PacketQueue* MacQueue::GetPacketQueue(void) const {
  return m_queue;
}

bool MacQueue::Enqueue(Packet* packet) {
  ...
  std::cout << "MAC_DEBUG: Enqueue(), packetSize = " << element.GetSize()
            << " queueSize = " << GetQueueSize() << std::endl;
#endif

  return true;
}

Packet* MacQueue::GetPacketToTramsit(int availableBytes) {
#ifdef MAC_QUEUE_DEBUG
  std::cout << "MAC_DEBUG: Dequeue() availableBytes: " << availableBytes
            << " queueSize: " << GetQueueSize() << std::endl;
#endif
  ...
  if (dataToSend + overhead > availableBytes) {
    ...
    GetPacketQueue()->begin()->SetFragmentNumber(element.GetFragmentNumber() +
                                                 1);
...
void MacQueue::Dequeue() {
  GetPacketQueue()->pop_front();
  m_nbDataPackets -= 1;
}

MacQueue::QueueElement MacQueue::Peek(void) const {
  MacQueue::QueueElement element = *GetPacketQueue()->begin();
  return element;
}

bool MacQueue::IsEmpty(void) const {
...
Packet* MacQueue::QueueElement::GetPacket(void) {
  return m_packet;
}

int MacQueue::QueueElement::GetSize(void) const {
  return m_packet->GetSize();
}

double MacQueue::QueueElement::GetTimeStamp(void) const {
  return m_packet->GetTimeStamp();
}

void MacQueue::QueueElement::SetFragmentation(bool flag) {
  m_fragmentation = flag;
}

bool MacQueue::QueueElement::GetFragmentation(void) const {
  return m_fragmentation;
}

void MacQueue::QueueElement::SetFragmentNumber(int fragmentNumber) {
  m_fragmentNumber = fragmentNumber;
}

int MacQueue::QueueElement::GetFragmentNumber(void) const {
  return m_fragmentNumber;
}

void MacQueue::QueueElement::SetFragmentOffset(int fragmentOffset) {
  m_fragmentOffset += fragmentOffset;
}

int MacQueue::QueueElement::GetFragmentOffset(void) const {
  return m_fragmentOffset;
}

void MacQueue::QueueElement::SetTempFragmentNumber(int fragmentNumber) {
  m_tempFragmentNumber = fragmentNumber;
}

int MacQueue::QueueElement::GetTempFragmentNumber(void) const {
  return m_tempFragmentNumber;
}

void MacQueue::QueueElement::SetTempFragmentOffset(int fragmentOffset) {
  m_tempFragmentOffset += fragmentOffset;
}

int MacQueue::QueueElement::GetTempFragmentOffset(void) const {
  return m_tempFragmentOffset;
}

void MacQueue::ModifyPacketSorceID(int id) {
...
void MacQueue::CheckForDropPackets(double maxDelay, int bearerID) {
    ...
    if (headOfLineDelay > maxDelay) {
      ...
      if (_MAC_TRACING_) {
        ...
        if (GetPacketQueue()
                ...
                ->GetPacket()
                ->GetPacketTags()
                ->GetApplicationType() == PacketTAGs::APPLICATION_TYPE_VOIP)
          std::cout << " VOIP";
        else if (GetPacketQueue()
                     ->begin()
                     ->GetPacket()
                     ->GetPacketTags()
                     ->GetApplicationType() ==
          ...
          std::cout << " VIDEO";
                     ...
                     ->GetPacket()
                     ->GetPacketTags()
                     ->GetApplicationType() == PacketTAGs::APPLICATION_TYPE_CBR)
          std::cout << " CBR";
        else if (GetPacketQueue()
                     ->begin()
                     ->GetPacket()
      ...
      int size = GetPacketQueue()->begin()->GetSize() -
                 GetPacketQueue()->begin()->GetFragmentOffset();
...
void MacQueue::PrintQueueInfo(void) {
  std::cout << "\t ** queue info: "
               "\n\t ** total size = "
            << GetQueueSize() << " packets = " << GetNbDataPackets()
            << std::endl;
  ...
  for (iter = queue->begin(); iter != queue->end(); iter++) {
    std::cout << "\t ** pkt --> "
                 " size  "
              << iter->GetSize() << " offset " << iter->GetFragmentOffset()
              << std::endl;
  }
}

int MacQueue::GetByte(int byte) {
  PacketQueue* queue = GetPacketQueue();
  PacketQueue::iterator iter;
