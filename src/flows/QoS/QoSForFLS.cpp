...
void QoSForFLS::CreateFilterCoefficients(void) {
  ...
  for (int i = 2; i < m_nbOfCoefficients + 1; i++) {
    m_filterCoefficients[i] = m_filterCoefficients[i - 1] / 2;
  }

#ifdef SCHEDULER_DEBUG
  std::cout << " CreateFilterCoefficients for Bearer " << GetBearerID()
            << std::endl;
  for (int i = 0; i < m_nbOfCoefficients + 1; i++) {
...
void QoSForFLS::SetNbOfCoefficients(int M) {
...
int QoSForFLS::GetNbOfCoefficients(void) const {
  return m_nbOfCoefficients;
}

int* QoSForFLS::GetQ(void) {
  return m_q;
}

int* QoSForFLS::GetU(void) {
  return m_u;
}

double* QoSForFLS::GetFilterCoefficients(void) const {
  return m_filterCoefficients;
}

void QoSForFLS::UpdateQ(int q_) {
#ifdef SCHEDULER_DEBUG
...
void QoSForFLS::SetDataToTransmit(int data) {
  m_dataToTransmit = data;
}

int QoSForFLS::GetDataToTransmit(void) const {
  return m_dataToTransmit;
}

void QoSForFLS::UpdateDataToTransmit(int data) {
  m_dataToTransmit -= data;
