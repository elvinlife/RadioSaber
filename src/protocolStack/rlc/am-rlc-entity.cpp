...
void AmRlcEntity::ClearPacketList(void) {
...
void AmRlcEntity::PrintSentAMDs(void) {
  std::cout << "\t PrintSentAMDs" << std::endl;
  std::vector<AmdRecord*>::iterator it;
  for (it = m_sentAMDs->begin(); it != m_sentAMDs->end(); it++) {
    Packet* packet = (*it)->m_packet;
    std::cout << "\t\t *** pkt " << packet->GetID() << " frag "
              << packet->GetRLCHeader()->GetFragmentNumber() << " sn "
              << packet->GetRLCHeader()->GetRlcPduSequenceNumber() << " startB "
              << packet->GetRLCHeader()->GetStartByte() << " endB "
              << packet->GetRLCHeader()->GetEndByte() << std::endl;
  }
}

void AmRlcEntity::PrintReceivedAMDs(void) {
...
AmStateVariables* AmRlcEntity::GetAmStateVariables() {
  return m_amStateVariables;
}

std::vector<AmdRecord*>* AmRlcEntity::GetSentAMDs(void) {
  return m_sentAMDs;
}

std::vector<AmdRecord*>* AmRlcEntity::GetReceivedAMDs(void) {
  return m_receivedAMDs;
}

void AmRlcEntity::InsertAMDIntoReceptionList(AmdRecord* amd) {
#ifdef RLC_DEBUG
  ...
  for (int i = 0; i < amds; i++) {
    ...
    if (amd->m_packet->GetRLCHeader()->GetRlcPduSequenceNumber() <
      ...
      return;
    } else if (amd->m_packet->GetRLCHeader()->GetRlcPduSequenceNumber() >
                   packet->GetRLCHeader()->GetRlcPduSequenceNumber() &&
               i == amds - 1) {
      ...
      return;
               ...
               packet->GetRLCHeader()->GetRlcPduSequenceNumber()) {
      if (amd->m_packet->GetRLCHeader()->GetEndByte() <
          packet->GetRLCHeader()->GetStartByte()) {
...
PacketBurst* AmRlcEntity::TransmissionProcedure(int availableBytes) {
#ifdef RLC_DEBUG
  std::cout << "AM RLC tx procedure for node "
            << GetRadioBearerInstance()->GetSource()->GetIDNetworkNode()
            << " bearer " << GetRlcEntityIndex() << std::endl;
#endif
  ...
  if (nbSentAMDs != 0) {
    ...
    while (availableBytes > 0 && amdId < nbSentAMDs) {
      ...
      if (amdRecord->m_packet->GetSize() + 6 <=
          availableBytes)  //6 = MAC  + CRC overhead
      {
        ...
        if (_RLC_TRACING_) {
          std::cout << "TX AM_RLC SIZE" << p->GetSize() << " B "
                    << GetRlcEntityIndex() << " PDU_SN "
                    << p->GetRLCHeader()->GetRlcPduSequenceNumber()
                    << std::endl;
        }
      ...
      } else if (availableBytes > 8)  // 8 = RLC + MAC + CRC
      {
        ...
        amdRecord->m_retx_count++;
#ifdef RLC_DEBUG
        std::cout << "there is not enough bandwidth for this AMD PDU. Do "
        ...
        p1->GetRLCHeader()->SetEndByte(p1->GetRLCHeader()->GetStartByte() +
                                       sentBytes - 1);
        p1->SetSize(2 + sentBytes);  //consider both RLC and sent bytes
        p2->GetRLCHeader()->SetStartByte(p1->GetRLCHeader()->GetStartByte() +
                                         sentBytes);
        p2->SetSize(
            p2->GetSize() -
            sentBytes);  //decrease the packet size by the quota of sent bytes
        ...
        if (_RLC_TRACING_) {
          std::cout << "TX AM_RLC SIZE" << p1->GetSize() << " B "
                    << GetRlcEntityIndex() << " PDU_SN "
                    << p1->GetRLCHeader()->GetRlcPduSequenceNumber()
                    << std::endl;
        }

        pb->AddPacket(p1->Copy());

        AmdRecord* newAmdRecord =
            new AmdRecord(p1, p1->GetRLCHeader()->GetRlcPduSequenceNumber());
  ...
  if (bearer->GetApplication()->GetApplicationType() ==
    ...
    while (true) {
      ...
      if (availableBytes > 1503) {
        ...
        if (_RLC_TRACING_) {
          std::cout << "TX AM_RLC SIZE " << packet->GetSize() << " B "
                    << GetRlcEntityIndex() << " PDU_SN "
                    << packet->GetRLCHeader()->GetRlcPduSequenceNumber()
        ...
        if (_RLC_TRACING_) {
          std::cout << "TX 2 AM_RLC SIZE " << packet->GetSize() << " B "
                    << GetRlcEntityIndex() << " PDU_SN "
                    << packet->GetRLCHeader()->GetRlcPduSequenceNumber()
                    << " Frag " << packet->GetRLCHeader()->IsAFragment()
                    << " LastFrag "
...
#ifdef RLC_DEBUG
        std::cout << "SEND PACKET id " << packet->GetID() << " frag n "
                  << packet->GetRLCHeader()->GetFragmentNumber() << std::endl;
#endif
...
#ifdef RLC_DEBUG
        std::cout << "_____ pkt " << packet->GetID() << " frag "
                  << packet->GetRLCHeader()->GetFragmentNumber() << " sn "
                  << packet->GetRLCHeader()->GetRlcPduSequenceNumber()
                  << " startB " << packet->GetRLCHeader()->GetStartByte()
                  << " endB " << packet->GetRLCHeader()->GetEndByte()
...
void AmRlcEntity::ReceptionProcedure(Packet* p) {
#ifdef RLC_DEBUG
  std::cout << "AM RLC rx procedure for node "
            << GetRadioBearerInstance()->GetDestination()->GetIDNetworkNode()
            << " bearer " << GetRlcEntityIndex() << std::endl;
  std::cout << "_____ pkt " << p->GetID() << " frag "
            << p->GetRLCHeader()->GetFragmentNumber() << " sn "
            << p->GetRLCHeader()->GetRlcPduSequenceNumber() << " startB "
            << p->GetRLCHeader()->GetStartByte() << " endB "
            << p->GetRLCHeader()->GetEndByte() << std::endl;
#endif

  if (_RLC_TRACING_) {
    std::cout << "RX AM_RLC SIZE " << p->GetSize() << " B "
              << GetRlcEntityIndex() << " PDU_SN "
...
void AmRlcEntity::ReceptionProcedureEnd() {
#ifdef RLC_DEBUG
  std::cout << "AM RLC ReceptionProcedureEnd " << std::endl;
  PrintReceivedAMDs();
#endif

  if (m_receivedAMDs->size() == 0)
    return;
  ...
  for (it = m_receivedAMDs->begin(); it != m_receivedAMDs->end(); it++) {
    ...
    if (packet->GetID() != currentPacket &&
        packet->GetRLCHeader()->GetStartByte() == 0) {
    ...
    } else if (packet->GetID() != currentPacket &&
               packet->GetRLCHeader()->GetStartByte() != 0) {
      //std::cout << "\t\t !! fragment of a new packet, but not he first" << std::endl;
      expectedNextByte = 0;
    } else if (packet->GetID() == currentPacket &&
               packet->GetRLCHeader()->GetStartByte() == expectedNextByte) {
  ...
  for (std::list<int>::iterator iter = packetIdToDelete.begin();
       iter != packetIdToDelete.end(); iter++) {
    std::vector<AmdRecord*>* newAmdList = new std::vector<AmdRecord*>;
    int id = (*iter);
    for (std::vector<AmdRecord*>::iterator itt = m_receivedAMDs->begin();
         itt != m_receivedAMDs->end(); itt++) {
      AmdRecord* amdRecord = (*itt);
      double delay =
          amdRecord->m_packet->GetTimeStamp() +
          GetRadioBearerInstance()->GetQoSParameters()->GetMaxDelay();
...
#ifdef RLC_DEBUG
  std::cout << "\t\t !! DELETE PACKETS or AMDs WHOSE DEADLINE is EXPIRED !"
            << std::endl;
#endif
  ...
  for (std::vector<AmdRecord*>::iterator itt = m_receivedAMDs->begin();
       itt != m_receivedAMDs->end(); itt++) {
    AmdRecord* amdRecord = (*itt);
    double delay = amdRecord->m_packet->GetTimeStamp() +
                   GetRadioBearerInstance()->GetQoSParameters()->GetMaxDelay();
...
void AmRlcEntity::SendArqRlcIdealControlMessage(ArqRlcIdealControlMessage msg) {
#ifdef RLC_DEBUG
  std::cout << "AM RLC -- SendArqRlcIdealControlMessage, node "
            << GetRadioBearerInstance()->GetDestination()->GetIDNetworkNode()
            << " bearer " << GetRlcEntityIndex() << std::endl;
  std::cout << "\t ack " << msg.GetAck() << ", startB " << msg.GetStartByte()
            << ", endB " << msg.GetEndByte() << std::endl;
#endif
  ...
  ApplicationSink* thisApplication =
      (ApplicationSink*)thisBearer->GetApplication();
  RadioBearer* remoteBearer =
      thisApplication->GetSourceApplication()->GetRadioBearer();

  AmRlcEntity* remoteRlc = (AmRlcEntity*)remoteBearer->GetRlcEntity();
  remoteRlc->ReceiveArqRlcIdealControlMessage(msg);
}

void AmRlcEntity::ReceiveArqRlcIdealControlMessage(
    ArqRlcIdealControlMessage msg) {
#ifdef RLC_DEBUG
  std::cout << "AM RLC -- ReceiveArqRlcIdealControlMessage, node "
            << GetRadioBearerInstance()->GetSource()->GetIDNetworkNode()
            << " bearer " << GetRlcEntityIndex() << std::endl;
  std::cout << "\t ack " << msg.GetAck() << ", startB " << msg.GetStartByte()
            << ", endB " << msg.GetEndByte() << std::endl;
  ...
  for (it = m_sentAMDs->begin(); it != m_sentAMDs->end(); it++) {
    ...
    if (amdRecord->m_packet->GetRLCHeader()->GetRlcPduSequenceNumber() ==
            msg.GetAck() &&
        amdRecord->m_packet->GetRLCHeader()->GetStartByte() ==
            msg.GetStartByte()) {
      m_sentAMDs->erase(it);
      delete amdRecord;
...
void AmRlcEntity::CheckForDropPackets(double maxDelay, int bearerID) {
#ifdef RLC_DEBUG
  ...
  for (std::vector<AmdRecord*>::iterator it = m_sentAMDs->begin();
       it != m_sentAMDs->end(); it++) {
    AmdRecord* amdRecord = (*it);
    ...
    if (HOL > maxDelay || amdRecord->m_retx_count > MAX_AMD_RETX) {
      if (currentpacket != amdRecord->m_packet->GetID()) {
        ...
        if (_RLC_TRACING_) {
           ...
           */
          std::cout << "DROP";
          if (amdRecord->m_packet->GetPacketTags()->GetApplicationType() ==
              PacketTAGs::APPLICATION_TYPE_VOIP)
            std::cout << " VOIP";
          else if (amdRecord->m_packet->GetPacketTags()->GetApplicationType() ==
                   PacketTAGs::APPLICATION_TYPE_TRACE_BASED)
            std::cout << " VIDEO";
          else if (amdRecord->m_packet->GetPacketTags()->GetApplicationType() ==
                   PacketTAGs::APPLICATION_TYPE_CBR)
          ...
          std::cout << " ID " << amdRecord->m_packet->GetID() << " B "
                    << GetRlcEntityIndex();
          if (amdRecord->m_packet->GetPacketTags() != NULL &&
              amdRecord->m_packet->GetPacketTags()->GetApplicationType() ==
                  PacketTAGs::APPLICATION_TYPE_TRACE_BASED) {
            std::cout << " FRAME "
                      << amdRecord->m_packet->GetPacketTags()->GetFrameNumber()
                      << " START "
                      << amdRecord->m_packet->GetPacketTags()->GetStartByte()
                      << " END "
                      << amdRecord->m_packet->GetPacketTags()->GetEndByte();
          }
          std::cout << std::endl;
        }
      }
...
int AmRlcEntity::GetSizeOfUnaknowledgedAmd(void) {
  int size = 0;
  for (std::vector<AmdRecord*>::iterator it = m_sentAMDs->begin();
       it != m_sentAMDs->end(); it++) {
