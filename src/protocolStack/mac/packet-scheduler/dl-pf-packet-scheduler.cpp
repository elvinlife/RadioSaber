...
void DL_PF_PacketScheduler::DoStopSchedule(void) {
  ...
  for (FlowsToSchedule::iterator it = flowsToSchedule->begin();
    ...
    if (availableBytes > 0) {
      ...
      flow->GetBearer()->UpdateCumulateRBs(
      ...
      std::cerr << GetTimeStamp() << " app: " << app_id
                << " cumu_bytes: " << flow->GetBearer()->GetCumulateBytes()
                << " cumu_rbs: " << flow->GetBearer()->GetCumulateRBs()
                << " hol_delay: "
                << flow->GetBearer()->GetHeadOfLinePacketDelay()
                << " user: " << user_id << " slice: " << user_to_slice_[user_id]
                << std::endl;
...
double DL_PF_PacketScheduler::ComputeSchedulingMetric(RadioBearer* bearer,
