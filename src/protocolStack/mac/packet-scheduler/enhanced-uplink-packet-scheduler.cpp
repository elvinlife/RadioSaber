...
void EnhancedUplinkPacketScheduler::RBsAllocation() {
  ...
  for (int j = 0; j < users->size(); j++) {
    ...
    requiredPRBs[j] =
        (floor)(scheduledUser->m_dataToTransmit /
  ...
  while (availableRBs > 0 && unallocatedUsers > 0)  //
  {
      ...
      if (!Allocated[i]) {  // check only unallocated PRB's
        for (int j = 0; j < users->size(); j++) {
          if (users->at(j)->m_listOfAllocatedRBs.size() == 0 &&
              requiredPRBs[j] >
                  0)  //only unallocated users requesting some RB's
    ...
    if (selectedUser != -1) {
      ...
      allocationMade = true;
      for (int i = 1;
           i < requiredPRBs[selectedUser] && availableRBs > 0 && allocationMade;
           i++) {  // search right and left of initial allocation
        ...
        if ((right < nbOfRBs) && (!Allocated[right]) &&
          ...
          availableRBs--;
                   ...
                   (((right < nbOfRBs) &&
                    ...
                    || (right >= nbOfRBs) ||
                    Allocated[right]  // OR no more right
                    )) {
...
#ifdef SCHEDULER_DEBUG
        printf(
            "Scheduled User = %d mcs = %d Required RB's = %d Allocated RB's= "
            "%d\n",
            scheduledUser->m_userToSchedule->GetIDNetworkNode(),
            scheduledUser->m_selectedMCS, requiredPRBs[selectedUser],
            scheduledUser->m_listOfAllocatedRBs.size());
