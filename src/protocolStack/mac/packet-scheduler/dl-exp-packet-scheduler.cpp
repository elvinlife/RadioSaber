...
double DL_EXP_PacketScheduler::ComputeSchedulingMetric(
  ...
  for (iter = flowsToSchedule->begin(); iter != flowsToSchedule->end();
    ...
    if (bearer->HasPackets()) {
      if ((bearer->GetApplication()->GetApplicationType() ==
        ...
        QoSForEXP* qos = (QoSForEXP*)bearer->GetQoSParameters();
        double aWi = -(log10(qos->GetDropProbability()) / qos->GetMaxDelay());
        double HOL = bearer->GetHeadOfLinePacketDelay();
        aWi = aWi * HOL;
        m_aW += aWi;
        nbFlow++;
...
double DL_EXP_PacketScheduler::GetAW(void) const {
  return m_aW;
}
