...
static void TestThroughputMacroWithFemto(double radius, int nbBuildings,
                                         int nbUE_macro) {
  ...
  for (int i = 0; i < nbUE_macro; i++) {
    ...
    UserEquipment* ue =
        new UserEquipment(idUE, posX, posY, speed, speedDirection,
                          nm->GetCellByID(j), nm->GetENodeBByCellID(j),
                          1,  //handover false!
  ...
  for (std::vector<UserEquipment*>::iterator it =
           nm->GetUserEquipmentContainer()->begin();
       it != nm->GetUserEquipmentContainer()->end(); it++) {
    UserEquipment* ue = (*it);
    if (ue->GetTargetNode()->GetNodeType() == NetworkNode::TYPE_ENODEB) {
      std::cout
          ...
          << std::endl;
    } else if (ue->GetTargetNode()->GetNodeType() ==
