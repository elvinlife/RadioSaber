...
static void ScalabilityTestMacroWithFemto(double radius, int nbBuildings,
                                          int nbUE_buildings) {
  srand(time(NULL));
  ...
  for (int i = 0; i < nbBuildings; i++) {
    ...
    nm->CreateBuildingForFemtocells(idBuilding, buildType, apartmentSide,
                                    nbFloors, x, y, idFemto,
                                    femtoCellsInBuilding);

    std::cout << "Building " << idBuilding << " position " << x << " " << y
  ...
  if (nbBuildings > 0) {
    for (int i = 0; i < nbBuildings; i++) {
      ...
      for (int k = 0; k < nbUE_buildings; k++) {
        ...
        double posX =
            henb->GetMobilityModel()->GetAbsolutePosition()->GetCoordinateX();
        double posY =
            henb->GetMobilityModel()->GetAbsolutePosition()->GetCoordinateY();
        double speed = 3;
        double speedDirection = (double)(rand() % 360) * ((2 * 3.14) / 360);

        UserEquipment* ue = new UserEquipment(
            idUE, posX, posY, speed, speedDirection, henb->GetCell(), henb, 0,
        ...
        MacroCellUrbanAreaChannelRealization* c_ul =
            new MacroCellUrbanAreaChannelRealization(ue, target);
        target->GetPhy()
            ->GetUlChannel()
            ->GetPropagationLossModel()
  ...
  for (int i = 0; i < macro_ue; i++) {
    ...
    UserEquipment* ue = new UserEquipment(
        idUE, x, y, speed, speedDirection, nm->GetCellByID(0),
        nm->GetENodeBByCellID(0), 0, Mobility::CONSTANT_POSITION);
    ENodeB* target = (ENodeB*)ue->GetTargetNode();
    if (target->GetNodeType() == NetworkNode::TYPE_HOME_BASE_STATION) {
      ue->SetIndoorFlag(true);
    }
