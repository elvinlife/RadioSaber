...
*/
      ...
      for (int j = 0; j < femtoCellsInBuilding; j++) {
        ...
        /*
           ...
           x = (apartmentSide*x) + enb->GetMobilityModel ()->GetAbsolutePosition
           ()->GetCoordinateX (); double y = (double)rand()/RAND_MAX; y =
           (apartmentSide*y) + enb->GetMobilityModel ()->GetAbsolutePosition
           ()->GetCoordinateY (); CartesianCoordinates *position = new
           CartesianCoordinates(x, y); enb->GetMobilityModel
  ...
  int idUE = 3001;
  for (double x = -25.; x < 25.; x = x + 1) {
    for (double y = -25.; y < 25.; y = y + 1) {
      double posX = x;
      double posY = y;
      double speed = 3;
      ...
      UserEquipment* ue = new UserEquipment(
          idUE, posX, posY, speed, speedDirection,
          nm->GetFemtoCellContainer()->at(0),
          nm->GetHomeENodeBContainer()->at(0), 1, Mobility::CONSTANT_POSITION);
      ...
      std::cout << "UE " << idUE << " position " << posX << " " << posY
                      ...
                      << " henb " << ue->GetTargetNode ()->GetIDNetworkNode ()
      << std::endl;
       */
