...
static void TestThroughputUrban(int streets, int henb, int reuse, int nbUE,
  ...
  for (int i = 0; i < nbCell; i++) {
    ...
    Cell* c = new Cell(i, radius / 1000., 0.035, center.GetCoordinateX(),
                       center.GetCoordinateY());
    nm->GetCellContainer()->push_back(c);

    std::cout << "CELL " << c->GetIdCell() << " position "
  ...
  if (streets > 0) {
    ...
    x = x_cross;
    y = y_cross + ((2 * apartment_side * 5 + interBuilding_distance) +
                   (nbOfBuildingInStreet / 2) *
                       (apartment_side * 5 + interBuilding_distance));
    theta = 90.;
    firstFemtoinBuildingID = 1001;

    nm->CreateStreet(1001, new CartesianCoordinates(x, y), theta, buildingtype,
                     nbOfBuildingInStreet, street_width, interBuilding_distance,
                     femtoCellsInBuilding, apartment_side,
                     firstFemtoinBuildingID);

    // create street 2
    nbOfBuildingInStreet = 6;
    x = x_cross + ((2 * apartment_side * 5 + interBuilding_distance) +
                   (nbOfBuildingInStreet / 2) *
                       (apartment_side * 5 + interBuilding_distance));
    y = y_cross;
    theta = 0.;
    firstFemtoinBuildingID = 2001;
    nm->CreateStreet(2001, new CartesianCoordinates(x, y), theta, buildingtype,
                     nbOfBuildingInStreet, street_width, interBuilding_distance,
                     femtoCellsInBuilding, apartment_side,
                     firstFemtoinBuildingID);
    ...
    nm->CreateStreet(3001, new CartesianCoordinates(x, y), theta, buildingtype,
                     nbOfBuildingInStreet, street_width, interBuilding_distance,
                     femtoCellsInBuilding, apartment_side,
                     firstFemtoinBuildingID);
    ...
    nm->CreateStreet(4001, new CartesianCoordinates(x, y), theta, buildingtype,
                     nbOfBuildingInStreet, street_width, interBuilding_distance,
                     femtoCellsInBuilding, apartment_side,
                     firstFemtoinBuildingID);
      ...
      for (int i = 0; i < femtocells->size(); i++) {
        ...
        if (rnd <= activityFactor) {
          ...
          if (reuse == 0) {
            ...
            enb->GetPhy()->SetBandwidthManager(
                new BandwidthManager(5, 5, 75, 75));
          }

          nm->GetHomeENodeBContainer()->push_back(enb);

          std::cout << "HeNB " << enb->GetIDNetworkNode() << " cell "
                    << enb->GetCell()->GetIdCell() << " position "
                    << enb->GetMobilityModel()
                           ->GetAbsolutePosition()
                           ->GetCoordinateX()
                    << " "
                    << enb->GetMobilityModel()
  ...
  for (int i = 0; i < nbUE; i++) {
    ...
    posX = 0.90 * (((2 * radius) * posX) - radius);
    double posY = (double)rand() / RAND_MAX;
    posY = 0.90 * (((2 * radius) * posY) - radius);
    double speed = 3;
    double speedDirection = (double)(rand() % 360) * ((2 * 3.14) / 360);

    UserEquipment* ue = new UserEquipment(
        idUE, posX, posY, speed, speedDirection, nm->GetCellContainer()->at(0),
        nm->GetENodeBContainer()->at(0), 0, Mobility::CONSTANT_POSITION);
    ...
    std::cout << "UE " << idUE << " position " << posX << " " << posY
              << " cell " << ue->GetCell()->GetIdCell() << " target "
