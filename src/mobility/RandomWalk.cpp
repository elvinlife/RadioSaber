...
void RandomWalk::UpdatePosition(double time) {
  ...
  if (GetSpeed() == 0) {
#ifdef MOBILITY_DEBUG
    std::cout << "\t\t speed = 0 --> position has not been updated!"
              << std::endl;
#endif
    return;
  }
  ...
  std::cout << GetNodeID() << "" << GetAbsolutePosition()->GetCoordinateX()
            << " " << GetAbsolutePosition()->GetCoordinateY() << " "
            << GetSpeed() << " " << GetSpeedDirection() << " "
            << GetPositionLastUpdate() << " " << timeInterval << " "
            << std::endl;
#endif
  ...
  CartesianCoordinates* newPosition = new CartesianCoordinates(
      GetAbsolutePosition()->GetCoordinateX() + shift_x,
  ...
  if (newDistanceFromTheENodeB <= (thisCell->GetMinDistance() * 1000)) {
    CartesianCoordinates* Correction = new CartesianCoordinates(
        ((thisCell->GetMinDistance() * 1000) - newDistanceFromTheENodeB) *
            cos(azimut),
        ((thisCell->GetMinDistance() * 1000) - newDistanceFromTheENodeB) *
            sin(azimut));

    newPosition->SetCoordinateX(newPosition->GetCoordinateX() +
                                Correction->GetCoordinateX());
    newPosition->SetCoordinateY(newPosition->GetCoordinateY() +
                                Correction->GetCoordinateY());

    if ((azimut > GetSpeedDirection() - pi / 2) &&
        (azimut < GetSpeedDirection() + pi / 2)) {
    ...
    if (GetHandover() == false) {
      ...
      CartesianCoordinates* Correction = new CartesianCoordinates(
          (newDistanceFromTheENodeB - (thisCell->GetRadius() * 1000)) *
              cos(azimut),
          (newDistanceFromTheENodeB - (thisCell->GetRadius() * 1000)) *
              sin(azimut));
  ...
  std::cout << "\n\t Final Position (X): "
            << GetAbsolutePosition()->GetCoordinateX()
            << "\n\t Final Position (Y): "
            << GetAbsolutePosition()->GetCoordinateY() << std::endl;
#endif
#ifdef MOBILITY_DEBUG_TAB
