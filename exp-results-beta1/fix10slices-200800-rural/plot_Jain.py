...
def get_JIndex_array(fname, n_users):
    ...
    rbs_2d = [cumu_rbs[i:i+n_users//slice_number] for i in range(0, len(cumu_rbs), n_users//slice_number)]
    J_index_array = []
    ...
    with open(fname, "r") as fin:
        for line in fin:
            ...
            if int(words[0]) > begin_ts:
                ...
                cumu_rbs[flow] = int( words[6] )
                # cumu_bytes[flow] = int( words[4] )
    slice_rbs = [0 for _ in range(slice_number)]
    for i in range(slice_number):
        for j in range(n_users//slice_number):
            slice_rbs[i] += cumu_rbs[i*(n_users//slice_number)+j]
...
def plot_intra_JainIndex():
    ...
    fig.suptitle("fixing 10 slices-200800")

    dnames = ['4ues', '10ues', '20ues', '32ues']
    ues_array = [4, 10, 20, 32]

    x1_array = np.arange(10)
    x2_array = np.arange(10)
    x3_array = np.arange(10)
    x4_array = np.arange(10)

    nvs_mean, nvs_err, greedy_mean, greedy_err, subopt_mean, subopt_err = get_JainsIndex(dnames[0], ues_array[0]*10)
    ...
    axs[0,0].title.set_text("4ues")
    axs[0,0].legend(loc="lower left")

    nvs_mean, nvs_err, greedy_mean, greedy_err, subopt_mean, subopt_err = get_JainsIndex(dnames[1], ues_array[1]*10)
    ...
    axs[0,1].title.set_text("10ues")

    nvs_mean, nvs_err, greedy_mean, greedy_err, subopt_mean, subopt_err = get_JainsIndex(dnames[2], ues_array[2]*10)
    ...
    axs[1,0].title.set_text("20ues")

    nvs_mean, nvs_err, greedy_mean, greedy_err, subopt_mean, subopt_err = get_JainsIndex(dnames[3], ues_array[3]*10)
...
def plot_inter_JainIndex():
    ...
    fig.suptitle("fixing 10 slices-200800 InterSlice Jain Index")
    x_array = [4,10,20,32]
    y_array_nvs = []
    y_array_greedy = []
    y_array_subopt = []
    y_array_single = []
    for i in range(4):
        nvs_index, greedy_index, subopt_index, single_index = get_inter_JainsIndex(dnames[i], ues_array[i]*10)
